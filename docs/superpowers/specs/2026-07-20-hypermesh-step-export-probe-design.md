...
已确认 `hm_getboundingbox components ...` 对当前 CAD Surface 模型会报告 `No entities found`，因此不能直接把 Component 作为包围盒实体。实现必须先构造属于该 Component 的几何实体 mark，再计算包围盒。

### 6.3 显示状态与导出

运行开始时保存所有当前显示的 Component ID。每次导出执行：
